...
## Config
- Copy `.env.example` → `.env.production` (production; gitignored) and `.env.test.example` → `.env.test` (local dev + tests; `.env.test` is gitignored). `DATABASE_URL` + `JWT_SECRET`/`JWT_REFRESH_SECRET` are required at boot.
- Env selection: `ConfigModule` in `src/app.module.ts` loads `.env.test` unless `NODE_ENV=production`, in which case it loads `.env.production`. Jest sets `NODE_ENV=test` automatically, so unit + e2e suites use `.env.test`. Local `npm run start:dev` also uses `.env.test`; `npm run start:prod` sets `NODE_ENV=production` and reads `.env.production`.
- Prisma CLI/scripts (`prisma.config.ts`, `lib/prisma.ts`, `scripts/verify-prisma.ts`) read `.env.production` via `dotenv/config` when `DOTENV_CONFIG_PATH=.env.production`; for prod DB work use `db:migrate:prod` / `db:seed:prod` / `db:verify:prod`, for local/test work use the `db:*:test` scripts.
